...
intercept <- F
for(conc in list(c(1,2), c(3,6), c(10,20))){ # not including a run with all 
  #concentrations since the data is too large, need to learn better memory 
  #management techniques for that.
  
  # This script sees if the fit improves using the predicted targets instead of 
  # the targets we have. Hence using predicted scripts instead of original ones.
  ...
  #CellNum sequence
  sequence <- 2^seq(0,10,1)
  
  #CellSize sequence
  #sequence <- c(seq(1, 2, 0.1), seq(3, 9, 1), seq(10, 90, 10), seq(100, 900, 100))#, seq(1000, 8000, 1000))#, 2^(13:30))
  
  Model_stats <- vector('list', length(sequence))
  names(Model_stats) <- as.character(sequence)
  ...
  pdf(paste0('./ModelStats/CellNum/Predicted/glmnet_Grps_Penalty', '_alpha:', alpha, 
  ...
  for (p in sequence){
    ...
    Model_stats[[as.character(p)]] <- list(lambda = fit$lambda, cvm = fit$cvm, 
                                           coef.min = coef(fit, 'lambda.min'),
                                           coef.1se = coef(fit, 'lambda.1se'),
    ...
    if (min(fit$cvm) == min(trainingTable$MSE, na.rm = T)){
      OptimalModel <- fit
    }
    gc()
  }
  ...
  for (penalty in sequence){
    tmp <- cbind(rep(penalty, length(Model_stats[[as.character(penalty)]]$lambda)), 
                 Model_stats[[as.character(penalty)]]$lambda, Model_stats[[as.character(penalty)]]$cvm)
    
    PenaltySpace <- rbind(PenaltySpace, tmp)
    
    tmp2 <- cbind(rep(penalty, length(Model_stats[[as.character(penalty)]]$coef.min[-1])), 
                  Model_stats[[as.character(penalty)]]$coef.min[-1],
  ...
  pdf(paste0('./ModelStats/CellNum/Predicted/lambdaError', '_alpha:', alpha, '_Int:', intercept,
  ...
  if (alpha == 0){
    ...
    for (i in 1:5){
      ...
      tmpfit <- glmnet(tmpPred, tmpOutcomes$CellNum, family = 'gaussian', 
                       ...
                       intercept = intercept, 
                       lambda = exp(seq(log(Model_stats[[as.character(minPenalty[[1]])]]$lambda.1se), 
                                        log(Model_stats[[as.character(minPenalty[[1]])]]$lambda.min), 
      ...
      predictions.min[[i]] <- predict(tmpfit, newx = newx, s = Model_stats[[as.character(minPenalty[[1]])]]$lambda.min)
      predictions.1se[[i]] <- predict(tmpfit, newx = newx, s = Model_stats[[as.character(minPenalty[[1]])]]$lambda.1se)
  ...
  write_csv(AllCoefs.min, 
            path = paste0('/data/Jaryd/R/CellSummerProject/Coefs/Penalty/CellNum/Predicted/', 
...
#      
#      
write_csv(ModelResults, paste0('./ModelStats/CellNum/Predicted/ModelResultsGrpdfixedPenaltyCellNum_rmvdups', Sys.Date(), file_suffix, '.csv'))

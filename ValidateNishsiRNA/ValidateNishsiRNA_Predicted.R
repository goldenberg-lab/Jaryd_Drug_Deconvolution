...
# no NA because I haven't optimized code to handle the larger size data required
# when all concentrations together.
grps1 <- read_csv('./grouping/latestgroupsPredicted1,2.csv') %>%
  select(gene_sym, group_sym) %>% dplyr::rename(group_sym1 = group_sym)
grps3 <- read_csv('./grouping/latestgroupsPredicted3,6.csv') %>%
  select(gene_sym, group_sym) %>% dplyr::rename(group_sym3 = group_sym)
grps10 <- read_csv('./grouping/latestgroupsPredicted10,20.csv') %>%
  select(gene_sym, group_sym) %>% dplyr::rename(group_sym10 = group_sym)

val <- left_join(val, grps1, by = 'gene_sym') %>%
...
CellNum_files <- get_coef_files('./Coefs/Penalty/CellNum/Predicted/')
#CellSize_files <- get_coef_files('./Coefs/Penalty/CellSize/Predicted/')

CellNum_files <- CellNum_files[grep('concs:10,20', CellNum_files)]
...
all_data <- left_join(all_data, CellNum_coefs %>%
                        ...
                        select(group_sym, contains('CellNum')),
                      by = c('gene_sym' = 'group_sym'))
...
write_csv(cors, './ValidateNishsiRNA/Correlation_Table_Predicted.csv')
...
ggsave('./graphs/For Paper/PDFs/CellNumValidationCorrelation_Predicted.pdf')
ggsave('./graphs/For Paper/PNGs/CellNumValidationCorrelation_Predicted.png')


pdf('./ValidateNishsiRNA/correlation_plots_Predicted.pdf')
...
# Plot Original CN coefs against Predicted CN coefs
orig_coef <- read_csv(paste0('./Coefs/Penalty/CellNum/CellNumMinDrgs:4_alpha:0',
...
grps10 <- read_csv('./grouping/latestgroups10,20.csv') %>%
  select(gene_sym, group_sym) %>% dplyr::rename(group_sym10.Orig = group_sym)

tmp <- left_join(TBP, grps10, by = 'gene_sym') %>%
  mutate(group_sym10.Orig = if_else(is.na(group_sym10.Orig), gene_sym, group_sym10.Orig))

TBP2 <- left_join(tmp, orig_coef, by = c("group_sym10.Orig" = "group_sym")) %>%
  dplyr::rename(Original_Coef = x, Predicted_Coef = coef)

library(ggrepel)
ggplot(TBP2[grep('CellNum.*Inter:F.*1se', TBP2$Model),], aes(x = Original_Coef, y = Predicted_Coef, label = gene_sym)) + 
  geom_point() + 
  geom_text_repel()
ggsave('./ValidateNishsiRNA/Orig_v_Pred_Coefs.pdf')

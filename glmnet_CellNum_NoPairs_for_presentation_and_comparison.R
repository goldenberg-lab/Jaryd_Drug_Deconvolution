...
for(intercept in c(T, F)){
  for(conc in list(NA, c(1,2), c(3,6), c(10,20))){
    source('import_data.R')
    source('glmnetHelperFuns.R')
    # added CellNum suffix to double check the groupings are always being 
    # calculated the same if doubted.
    lst <- import_grpd(average_replicates = avg_reps, rmv_cell_count_2 = F, 
                       create_group_files = F, two_conc = conc,
                       min_num_drugs = min_drugs, add_pair_features = pairs,
                       file_suffix = paste0(paste0(conc, collapse = ','), 'CellNum'))
    ...
    Model <- cv.glmnet(x = Predictors, y = Outcomes$CellNum,
                       alpha = alpha, family = "gaussian", foldid = Outcomes$bin,
                       intercept = intercept)
    
    pdf(paste0('./TestDeleteMeglmnetplot_Conc:', 
    ...
    #           path = paste0('/data/Jaryd/R/CellSummerProject/Coefs/Penalty/CellSize/', 
...
#      
#      
#write_csv(ModelResults, paste0('./ModelStats/CellSize/ModelResultsGrpdfixedPenaltyCellSize', Sys.Date(),'.csv'))

...
GeneManiaGrouping <- function(Network, GeneManiaDistance = 1, 
  ...
  Gene_ids <- unique(c(GeneNetwork[['ortho_id1']], GeneNetwork[['ortho_id2']]))
  ...
  Physical_Interactions <- read_tsv('../../CellSummerProjectData/GeneMania/Physical_interactions_12-03-2017.tsv', #Changed here
                                    col_types = cols())
  ...
  GMClassifiedCLEdges <- UnclassifiedCLEdges %>% rowwise() %>%
    mutate(ortho_id1 = as.character(ortho_id1), ortho_id2 = as.character(ortho_id2)) %>%
    mutate(neighbour = list(names(neighbours[[ortho_id1]]))) %>%
    mutate(GMConnection = ortho_id2 %in% neighbour)
  ...
  tmp <- GeneNetwork %>% mutate(ortho_id1 = as.character(ortho_id1), ortho_id2 = as.character(ortho_id2))

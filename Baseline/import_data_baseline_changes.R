...
import_grpd <- function(keep_all_genes = F, min_num_drugs = 4, 
                        ...
                        two_conc = NA, average_replicates = F, file_suffix = '',
  ...
  #####
  source('isoform grouping functions only.R')
  ...
  ActivityOrigin <- PlateBookID %>% select(Plate, Well, inchi_key, gene_id, activity_moa, origin)
  
  PlateBookID <- PlateBookID %>% select(-activity_moa, -origin)
  ...
  JoinedTable <- JoinedTable %>% filter(!is.na(inchi_key)) %>%
    filter(!gene_id %in% c(168448, 51716, 392360, 1578, 79427, 129870, 390003,
                          390226, 2974))
  
  if (only_primary_targets){
    primaryTargs <- read_csv(paste0('./post fit analysis/Evaluate_Primary_and_off_Target/',
                                    'PrimaryTargetTbl_', paste0(conc, collapse = ','), '.csv')) %>%
  ...
  # primary_targets_tbl already grouped, grouping groups is not good.
  if (!only_primary_targets){ 
    # calculate num_drugs per ortho_id for grouping purposes.
    JoinedTable <- JoinedTable %>% group_by(ortho_id) %>% 
  ...
  if (add_pair_features){
    ...
    pdf("pairedFeatureHistogram.pdf")
    print(histogram)
    dev.off()
  } else{
